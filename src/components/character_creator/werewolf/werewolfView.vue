...
export default defineComponent({
  ...
  async created() {
    ...
    const folder_res = await this.$api.get(
      "/char_folders/my_folders?game_line=2",
      {
        withCredentials: true,
      }
    );
    ...
    const chars_folders = await this.$api.get(
      `/char_folders/this_char?game_line=2&char_id=${char_id}`
    );
    ...
    async modifyPdf() {
      try {
        ...
        for (
          let i = 1;
          i <
          this.garou.attributes.composure + this.garou.attributes.resolve + 1;
          i++
        ) {
